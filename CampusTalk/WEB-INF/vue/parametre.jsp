...
<head>
    ...
    <title>CampusTalk - Paramètres du Compte</title>
...
<body class="bg-gray-900 text-white flex flex-col md:flex-row">
    ...
    <div class="flex-1 p-6">
        <div class="max-w-md mx-auto mt-12 p-6 bg-gray-800 rounded-lg shadow-md">
            <h2 class="text-2xl font-bold mb-4 text-center">Paramètres du Compte</h2>
            <% if (request.getAttribute("error") != null) { %>
                <div class="bg-red-100 text-red-700 p-4 rounded-lg mb-4">
                    <%= request.getAttribute("error") %>
                </div>
                <% request.removeAttribute("error"); } %>
            <form action="<%= request.getContextPath() %>/parametre" method="post">
                <div class="mb-4">
                    <label for="nom" class="block text-gray-300 font-semibold">Nom</label>
                    <input type="text" class="w-full p-3 border border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 bg-gray-700 text-white" name="nom" id="nom" value="<%= utilisateur.getNom() %>" required>
                </div>
                <div class="mb-4">
                ...
                <div class="mt-4 text-center">
                    <a href="<%= request.getContextPath() %>/logout" class="inline-block bg-red-500 text-white px-4 py-2 rounded-lg shadow-md hover:bg-red-600 transition-colors">Se déconnecter</a>
                </div>
            </form>
            <div class="mt-6 bg-gray-800 p-6 rounded-lg shadow-md">
                <h3 class="text-xl font-bold mb-4 text-center text-red-500">Supprimer votre compte</h3>
                <form action="<%= request.getContextPath() %>/deleteAccount" method="post" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer votre compte ? Cette action est irréversible.')">
                    <button type="submit" class="bg-red-500 text-white px-4 py-2 rounded-lg shadow-md hover:bg-red-600 transition-colors w-full">
